...
module.exports = {
    ...
    options: [
        ...
        {
            name: "log-kanalı",
            description: "Uptime sisteminin log kanalını ayarlarsınız.",
            type: 7,
            required: true,
            channel_types: [0]
        },
    ...
    run: async (client, interaction) => {
      ...
      const kanal = interaction.options.getChannel('kanal')
        const log = interaction.options.getChannel('log-kanalı')
        ...
        louritydb.set(`uptimeSistemi_${interaction.guild.id}`, { kanal: kanal.id, rol: rol.id, log: log.id })
    }
